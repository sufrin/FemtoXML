...
{  protected XMLHandler consumer;
   ...
   public XMLScanner(XMLHandler consumer)       { setConsumer(consumer); }

   public void setConsumer(XMLHandler consumer) { this.consumer = consumer; }
   ...
   { nextToken();
     if (this.token!=token)
        throw new XMLSyntax(token+" expected; found "+this.token+" "+this.value);
   }
   private enum Lex
   { ENDSTREAM          ("END-OF-XML-STREAM"),
     ...
     PROCESS            ("<? ... ?>"),
     DOCTYPE            ("<!DOCTYPE ...");
     Lex(String name) { this.name=name; }
     private String name;
     public String toString() { return name; }     
   }
   /** Read XML from the given Reader, invoking the current consumer's methods at appropriate times. */
   ...
   { this.reader    = reader;
     ...
     { switch (token) 
       { case DOCTYPE:
         ...
         default:
              throw new XMLSyntax("Unexpected token: "+token+" "+value);
         case POINTBRA:                         // <id id="..." ...
         {  Map<String,String> atts = new Attributes();
            ...
            { String key=value;
              ...
              throw new XMLSyntax("Found "+token+" when string expected in "+key+"=...");
            }
            ...
            if (token!=Lex.POINTKET)            // >
               throw new XMLSyntax("> expected in start tag: found "+token);
            inElement = false;
         }
       }
       nextToken();
   ...
   {  
    if (0<=ch && ch<=' ') { do { nextRawChar(); } while (0<=ch && ch<=' '); }
    ...
    { nextRawChar();
      ...
      throw new XMLSyntax("/> expected; found /"+((char) ch));
    }
    else
    if (ch=='<') 
    { nextRawChar();
      ...
      { nextRawChar();
        ...
        if (ch==-1)
           throw new XMLSyntax("<? with runaway body ...");
        else
        {
          nextRawChar();
      ...
      { 
        ...
        { StringBuilder b  = new StringBuilder();
          ...
          if (ch<0) 
             throw new XMLSyntax("<![CDATA[ ... ]]> expected; found <!"+b.toString()+" at end of file");
          else
          if (isCDATA(b))
          { 
          ...
          throw new XMLSyntax("<![CDATA[ ... ]]> expected; found <!"+b.toString()+">");
        }
        else
        if (ch=='D')    // Assume <!DOCTYPE and read to matching closing >
        { int count = 1;
          while (0<=ch && count>0)
          { nextRawChar();
            if (ch=='<') count++; else
          ...
          if (count!=0) throw new XMLSyntax("<!DOCTYPE with runaway body ...");
          token = Lex.DOCTYPE;
          nextRawChar();
        }
        else            // Assume <!-- comment -->
        { StringBuilder b  = new StringBuilder();
          ...
          throw new XMLSyntax("<!-- ... --> expected; found <!"+b.toString());
        }
      }
      else
        token=Lex.POINTBRA;
    ...
    else                        // a new ``word'' lump begins
    { StringBuilder b = new StringBuilder();
      token = Lex.IDENTIFIER;
      // leading & is a special case 
      if (ch=='&') { token=Lex.WORD; nextEnt(); b.append(entity); nextChar();  }
   ...
   protected void nextChar()  { nextRawChar(); if (ch=='&') { nextEnt(); ch='&'; } }
   ...
   /** Read and expand the next entity; the variable 'entity' is set to the expansion. */
   protected void nextEnt() 
   { String ent = "";
     nextRawChar();
     while (' '<ch && ch!=';') { ent=ent+((char) ch); nextRawChar(); }
     if (ent.equals("amp"))  entity = "&"; else
     if (ent.equals("ls"))   entity = "<"; else
     if (ent.equals("gr"))   entity = ">"; else
     if (ent.equals("apos")) entity = "'"; else
     if (ent.equals("quot")) entity = "\""; else
     if (ent.equals("nbsp")) entity = " "; else
     entity = consumer.decodeEntity(ent);
     if (entity==null) throw new XMLSyntax("Unknown entity: &"+ent+";");
   }
  ...
  { XMLHandler sax = new XMLHandler()
    {  void pr(CharSequence s, CharSequence t) { System.out.println(s+" "+t); }
       public void    wordCharacters(CharSequence chars)   { pr("WD", "'"+chars+"'"); };
       public void    startElement(String kind, Map<String,String> atts)  { pr("SE", kind+atts); };
       public void    endElement(String kind)              { pr("EE", kind); };
       public void    commentCharacters(CharSequence data) { pr("CO", data); };
       public void    startDocument()                      { pr("ST", ""); };
       public void    endDocument()                        { pr("EN", ""); };
       public String  decodeEntity(String s)               { return "&"+s+";"; };
    };
  ...
  { int           len    = s.length();
    ...
    for (int i=0; i<len; i++) 
    { char c = s.charAt(i);
      if (c==' ' || c=='&' || c=='>' || c=='<' || c=='"' || c=='\'' || (int) c>=128)
      { quoted = new StringBuilder(); break; }
    }
    if (quoted==null)
       return s;
    else
